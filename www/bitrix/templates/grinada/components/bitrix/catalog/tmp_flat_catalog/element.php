...
function get_real_section_id($code,$parent=0){
	...
	if($parent>0)$arF['SECTION_ID']=$parent;
	$id = array_shift(CIBlockSection::GetList(array(), $arF, false, array('ID') )->fetch());
	return $id;
}
function get_real_element_id($code, $parent){
	$iblock=7;
	$arF = array(
		'IBLOCK_ID' => $iblock,
		'CODE'=>$code,
	...
	//$id = array_shift();
	$res = CIBlockElement::GetList(array(), $arF, false, array('ID') );
	if($Item = $res->GetNext())return  $Item['ID'];
	//print_r($Item);
}
$arCodes = explode('/',$arResult['VARIABLES']['SECTION_CODE_PATH']);
$old = 0;
$real_section  = 0;
foreach($arCodes as $code_dir){
	$real_section = get_real_section_id($code_dir, $real_section);
}
